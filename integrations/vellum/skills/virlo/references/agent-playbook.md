...
- Check the balance with the free `GET /v1/account/balance` endpoint and surface it proactively when it dips below $10. (The `x-balance-remaining` response header is only present on charged responses — cost > 0 — so don't rely on it.)
