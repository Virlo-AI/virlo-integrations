...
- **Low balance:** check `GET /v1/account/balance` (free); when it's < $10, proactively warn. (Don't rely on the `x-balance-remaining` header — it's only present on charged responses, cost > 0.)
